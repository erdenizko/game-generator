  ...
  return (
    <Card className='p-0'>
      <CardHeader className='p-0 bg-transparent'>
        <img onClick={() => window.location.href = `/builder?gameId=${game.id}`} src={`https://placehold.co/400x600?text=${game.title}`} alt={game.title} className='w-full' />
        {/* <Image onClick={() => window.location.href = `/builder?gameId=${game.id}`} src={'https://placehold.co/600x400'} alt={game.title} width={100} height={100} className='w-full aspect-3/4 ' /> */}
      </CardHeader>
      <CardContent className='p-4 pt-60 text-white bg-gradient-to-b from-transparent via-black/60 to-black/80 absolute left-0 right-0 z-20 -translate-y-full'>
        <div className="flex justify-between items-start">
          <div onClick={() => window.location.href = `/builder?gameId=${game.id}`}>
            <CardTitle className="text-base flex flex-row gap-4">{game.title}</CardTitle>
        ...
        <div className="space-y-1 text-sm" onClick={() => window.location.href = `/builder?gameId=${game.id}`} >
          <div className="flex justify-between">
            <span>Columns</span>
