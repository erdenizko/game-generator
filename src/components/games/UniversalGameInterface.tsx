                        ...
                        <div className="flex items-center space-x-2">
                            {gameConfig.betAmounts.map(bet => (
                                <button
                                    ...
                                    className={`px-3 py-1 text-sm rounded transition-all ${selectedBet === bet
...
const ToastItem = ({ toast, onRemove }: { toast: ToastMessage, onRemove: () => void }) => {
    ...
    return (
        <div
            ref={toastRef}
            className={`p-3 rounded-lg shadow-lg text-white font-semibold ${toast.reward > 0 ? 'bg-green-600' : 'bg-red-600'
                }`}
        >
            <div className="flex items-center space-x-2">
